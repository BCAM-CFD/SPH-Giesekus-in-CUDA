...
int class_system::read_input(int   N[3],
  ...
  while (fgets(line, sizeof(line), file)) {
	...
	N_read             = 0;
      ...
      elast_time           = val1;
      elast_time_read      = 0;
    } else if (strcmp(var, "alpha") == 0 && count >= 2) {
      alpha_giesekus       = val1;
      alpha_giesekus_read  = 0;               
  ...
  if (alpha_giesekus_read == 1)  {
    printf("system_read_input error: alpha was not read\n");
    return 1;
  }      
