...
int main() {
  ...
  error = sys.read_input(Nxyz, Lbox, num_dim, delta_t, Nsteps,
			 ...
			 np, kT, elast_time, alpha_giesekus, 
			 ext_f_type, ext_f,
			 ...
			 coll_repulsion_cuton, F0_repulsion, tau_repulsion, new_sim);
  if (error != 0)
  ...
  error = sys.initialize(Nxyz, Lbox, num_dim, overlap, rho, c,
			 P0, eta, zeta, np, kT, elast_time, alpha_giesekus,
			 ext_f_type, ext_f, wall_host, Vwall_top, Vwall_bottom,
  ...
  cudaMemcpyToSymbol(alpha                , &sys.alpha                , sizeof(real));  
